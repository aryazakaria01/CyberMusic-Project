...
async def bye(client: Client, message: Message):
    if message.from_user.id in SUDO_USERS:
        ...
        if queryy == "on":
            PMSET = True
            await message.reply_text("**Pmpermit turn on**")
            return
        if queryy == "off":
            PMSET = None
            await message.reply_text("**Pmpermit turn off**")
            return
...
async def autopmPermiat(client: USER, message: Message):
    ...
    if chat_id not in pchats:
        ...
        await message.reply_text("Approve for Private Message")
        return
...
async def rmpmPermiat(client: USER, message: Message):
    ...
    if chat_id in pchats:
        ...
        await message.reply_text("Sorry you Disapprove for Private Message")
        return
    message.continue_propagation()    
